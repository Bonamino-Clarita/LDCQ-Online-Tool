document.addEventListener("DOMContentLoaded", () => {
  ...
  function showPage(index) {
    ...
    const questionnairePages = pages.filter((p) => p.classList.contains("questionnaire"));
    const questionnaireIndex = questionnairePages.indexOf(pages[index]);
    const progressLabel = document.getElementById("progress-label");
    const progressBar = document.getElementById("progress-bar");
    if (progressLabel && progressBar) {
      if (questionnaireIndex >= 0) {
        progressLabel.textContent = `Domain ${questionnaireIndex + 1} of ${questionnairePages.length}`;
        progressBar.style.width = `${((questionnaireIndex + 1) / questionnairePages.length) * 100}%`;
      } else if (pages[index].classList.contains("results-page")) {
